...
import '  /// Clear ship selection
  ...
  void _addTarget(UnitComponent unit) {
    if (!_targetedUnits.contains(unit)) {
      ...
      // Show feedback
      game.showUnitInfo("Target acquired: ${unit.model.type.toString().split('.').last}");
    }
  }
  ...
  bool handleEmptyTap(Vector2 worldPosition) {
    ...
    // If we have selected ships, move them
  ...
  /// Move selected ships to position
  ...
  void selectUnitsInBox(Vector2 screenStart, Vector2 screenEnd) {
    ...
    for (final unit in game.getAllUnits()) {
      ...
      if (!developmentMode && _playerTeam != null && unit.model.team != _playerTeam) {
        continue;
      }
    ...
    // Select ships
    for (final ship in game.getAllShips()) {
      if (ship.model.isDestroyed) continue;
      
      // DEVELOPMENT MODE: Allow selecting any ship
      // PRODUCTION MODE: Only select friendly ships
      if (!developmentMode && _playerTeam != null && ship.model.team != _playerTeam) {
        continue;
      }
      
      // Convert ship world position to screen position for selection
      final shipScreenPos = game.worldToScreenPosition(ship.position);
      
      if (shipScreenPos.x >= minX - selectionBuffer &&
          shipScreenPos.x <= maxX + selectionBuffer &&
          shipScreenPos.y >= minY - selectionBuffer &&
          shipScreenPos.y <= maxY + selectionBuffer) {
        ship.model.isSelected = true;
    ...
    // Set last selected ship if we selected any
    if (_selectedShips.isNotEmpty) {
      _lastSelectedShip = _selectedShips.first;
    }
  }
...
class UnitSelectionManager {
  ...
  void _addTarget(UnitComponent unit) {
    if (!_targetedUnits.contains(unit)) {
      _targetedUnits.add(unit);
      unit.model.isTargeted = true; // We'll need to add this property to UnitModel
